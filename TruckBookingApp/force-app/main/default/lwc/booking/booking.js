...
export default class Booking extends LightningElement {
    ...
    createBookingCase(event){
        createCases({bid:this.booking.Id, val:this.val}).then(data=>{
            var successToast=new ShowToastEvent({
                title:"Success",
                message:"Case has been created. We will contact you further!!",
                variant:"success",
                mode:"pester"
            });
            this.dispatchEvent(successToast);
            this.caseVisible=false;
            window.location.replace('https://mohitbam-dev-ed.lightning.force.com/lightning/n/Your_Cases');
        }).catch(error=>{
            var successToast=new ShowToastEvent({
                title:"Error",
                message:"Case has not been created!!",
                variant:"error",
                mode:"pester"
            });
            this.dispatchEvent(successToast);
